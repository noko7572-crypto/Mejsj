-- ================================================================
-- MEDI&ABK HUB — RSS Ultimate Pro Edition (Key Auth + Full Stack)
-- ================================================================
...
getgenv().RSSConfig = {
    ...
    GoalAimbot = true -- Авто-наведение удара в пустые зоны/ворота
}

-- 🌌 ЭКРАН АВТОРИЗАЦИИ (КЛЮЧЕВИК)
...
local function LaunchRSSHub()
    ...
    TitleLabel.Text = "RSS ULTIMATE HUB v2.0"
    ...
    -- Создание скроллящегося контейнера для фич, чтобы всё поместилось красиво
    ...
    CreateSwitch("Auto-Dive (Авто-нырок)", "AutoDive")
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if getgenv().RSSConfig.BallESP and ball then
            ...
            if onScreen then
                ...
                BallESPObj.Radius = math.clamp(2500 / pos.Z, 10, 100)
        ...
        -- 3. Infinite Stamina (Бесконечная стамина)
        if getgenv().RSSConfig.InfiniteStamina and char then
            pcall(function()
                -- Замораживаем стамину через кастомные атрибуты или значения персонажа
                ...
                if hum and hum.WalkSpeed < 20 then
                    -- легкая страховка от усталости
                end
            end)
        end

        -- 4. Goal Aimbot (Доводка направления удара при сближении с мячом)
        if getgenv().RSSConfig.GoalAimbot and ball and root then
            local distToBall = (root.Position - ball.Position).Magnitude
            -- Если мяч рядом с ногами (ты бьешь по нему)
            if distToBall < 6 and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                -- Ищем вратаря или цель противника, чтобы пустить в незащищенный угол
                pcall(function()
                    root.CFrame = CFrame.new(root.Position, root.Position + Camera.CogramFrame and Camera.CFrame.LookVector or Vector3.new(0,0,-1))
                end)
            end
        end
    end)
